...
class InverseLidarParallelParkingProblem(Problem):
    ...
    def _make_problem(self, env, rng=None):
        gap_start = rng.uniform(5.0, 10.0) * 2
        gap_size = 8
        ...
        # Random angle [-10°, 10°)
        start_angle = (rng.uniform() - 0.5) * 20 / 180 * np.pi

        start_pose = np.array(
            [
                gap_start + gap_size / 2 - street_length / 2 + rng.uniform(-0.5, 0.5),
                street_width / 2 + car_width / 2 + rng.uniform(-0.2, 0.2),
                start_angle,
            ]
        )
        self.start_pose_intervall_center = np.array(
            [-6.0, street_width / 2 + car_width / 2, 0.0]
        )[None, :]
        self.start_pose_intervall_tolerances = np.array([5.0, 0.2, 0.175])[None, :]

        target_x = 0.0 if self.start == "before" else 20 + gap_size + 5 + 0.5
        target_pose = np.array(
            [
                target_x - street_length / 2,
                0.0,
                0.0,
            ]
        )
    ...
    def configure_env(self, env, rng=None) -> Tuple[float, float, float]:
        ...
        # Make start_pose accessible to outside for reset success checking
