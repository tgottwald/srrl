...
def make_longitudinal_model(config: dict):
    from .Physics.VelocityController import (
        SimpleEngineDragVelocityController,
...
def make_problem(config: dict):
    from .Problems import (
        FreeDriveProblem,
        ParallelParkingProblem,
        LidarParallelParkingProblem,
        InverseLidarParallelParkingProblem,
        RacingProblem,
        RacingDuckProblem,
    )
    ...
    known_types = {
        ...
        "lidar_parallel_parking": LidarParallelParkingProblem,
        "inverse_lidar_parallel_parking": InverseLidarParallelParkingProblem,
        "racing": RacingProblem,
        "racing_duck": RacingDuckProblem,
    }
...
def parse_vehicle(config: dict):
    params = config.get("vehicle", {"type": "simple", "wheelbase": 2.4, "mass": 700.0})
...
class CarEnv(gym.Env):
    ...
    def __init__(self, config=None, render_mode=None, render_kwargs=None):
        ...
        self.__render_width, self.__render_height = render_kwargs.get(
            "width", 1280
        ), render_kwargs.get("height", 720)
        ...
        if self.render_mode == "human":
            ...
            self.__screen = pygame.display.set_mode(
                [rw, rh], flags=screen_flags, vsync=0
            )
        ...
        if self.render_mode in ["human", "rgb_array"]:
            ...
            prob_render_hints = (
                self.problem.render_hints
                if hasattr(self.problem, "render_hints")
                else {}
            )
            ...
            self.__renderer = BirdViewRenderer(
                self.__render_width, self.__render_height, **kwargs
            )
        ...
        if "lidar_points" in self._make_sensors().keys():
            low = np.concatenate(
                (
                    self.problem.state_observation_space.low,
                    self.sensors["lidar_points"].observation_space.low,
                ),
                axis=0,
            )
            high = np.concatenate(
                (
                    self.problem.state_observation_space.high,
                    self.sensors["lidar_points"].observation_space.high,
                ),
                axis=0,
            )

            self.observation_space = gym.spaces.Box(low, high, low.shape)
        else:
            obs_space = {
    ...
    def _render_impl(self, mode):
        if mode in ["human", "rgb_array"]:
            ...
            pygame.surfarray.blit_array(
                self.__screen, np.transpose(rgb_array, (1, 0, 2))
            )
            pygame.display.flip()
        elif mode is None:
            pass
    ...
    def render(self):
        if self.render_mode == "human":
            # Human mode already renders automatically, so assume that the caller wants a bitmap
            return self._render_impl("rgb_array")
        else:
            return self._render_impl(self.render_mode)
    ...
    def step(self, action) -> Tuple[Any, float, bool, bool, dict]:
        ...
        for _ in range(self.physics_divider):
            update_result = self.vehicle_model.update(
                control, self.dt / self.physics_divider
            )
        ...
        terminated = self._pending_termination or terminated
    ...
    def _make_obs(self):
        if "lidar_points" in self.sensors.keys():
            self.last_observations = self.problem.observe_state(self)
            self.last_observations = np.concatenate(
                (
                    self.last_observations,
                    self.sensors["lidar_points"].observe(self).flatten(),
                ),
                axis=0,
            )
        else:
    ...
    def _make_action(self):
        from .Actions import (
            ContinuousSteeringAction,
            HumanContinuousSteeringAccelAction,
            ContinuousSteeringAccelAction,
        )
        from .Actions import (
            HumanContinuousSteeringPedalsAction,
            ContinuousSteeringPedalsAction,
        )
    ...
    def _make_sensors(self):
        ...
        from .SensorDuckMap import SensorDuckMap
        from .SensorLidar import SensorLidar

        known_types = {
            "conemap": SensorConeMap,
            "duckmap": SensorDuckMap,
            "lidar": SensorLidar,
        }
    ...
    def show_agent_mode_indicator(self, show):
        if self.render_mode in ["human", "rgb_array"]:
            self.__renderer.draw_agent_mode_indicator = show
