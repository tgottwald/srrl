...
def main():
    ...
    parser.add_argument(
        "--env",
        choices=get_standard_env_names(),
        default="resetless_lidar_parking",
    )
    ...
    cfg["action"] = (
        {"type": "human"}
        if args.env == "parking"
        or args.env == "lidar_parking"
        or args.env == "resetless_lidar_parking"
        else {"type": "human_pedals"}
    )
    cfg["dt"] = 0.05
    env = CarEnv(
        cfg,
        render_mode="human",
        render_kwargs={"window": True, "hints": {"scale": 25.0}},
    )
    env.reset(seed=0)
